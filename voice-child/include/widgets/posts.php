	...
	function widget( $args, $instance ) {
		...
		<?php
			if ( $instance['style'] == 'vce-post-slider' && $vce_posts->post_count == 1 ) {
				$instance['style'] = 'vce-post-big'; //if only one post in slider, fallback to no slider
			}
?>

		<ul class="<?php echo esc_attr($instance['style']); ?>" data-autoplay="<?php echo esc_attr($instance['autoplay']);?>">
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['autoplay'] = !empty($new_instance['autoplay']) ? absint($new_instance['autoplay']) : '';
	...
	function form( $instance ) {
		...
		<p>
	  	...
	  	<?php $autoplay_display = $instance['style'] == 'vce-post-slider' ? 'block' : 'none'; ?>
	  	<p class="vce-autoplay-opt" style="display:<?php echo esc_attr( $autoplay_display ); ?>">
	   	 	<label for="<?php echo $this->get_field_id( 'autoplay' ); ?>"><?php _e( 'Autoplay slider:', THEME_SLUG ); ?> </label>
		 	<input id="<?php echo $this->get_field_id( 'autoplay' ); ?>" type="text" name="<?php echo $this->get_field_name( 'autoplay' ); ?>" value="<?php echo esc_attr( $instance['autoplay'] ); ?>" class="small-text" />
		 	<small class="howto"><?php _e( 'Specify number of seconds if you want to enable autoplay/rotate for slider', THEME_SLUG ); ?></small>
	  	</p>
...
?>
	...
	function widget_style( $widget_instance = false, $current = false ) {
		...
		if ( !empty( $widget_instance ) ) { ?>
				...
				<select id="<?php echo $widget_instance->get_field_id( 'style' ); ?>" name="<?php echo $widget_instance->get_field_name( 'style' ); ?>" class="widefat vce-post-widget-style">
